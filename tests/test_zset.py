...
from stepping.zset import _first_n, join
...
def to_zset(table: list[T]) -> ZSetPython[T]:
    zset = ZSetPython[T]()
    for row in table:
...
def test_index() -> None:
    ...
    zset = ZSetPython[int](indexes=(ix.generic,))
    zset += ZSetPython({3: 2})
...
def test_first_n() -> None:
    actual = list(_first_n(iter([(1, 1), (2, 1), (3, 1), (4, 1)]), 3))
    expected = [(1, 1), (2, 1), (3, 1)]
    assert actual == expected

    actual = list(_first_n(iter([(1, 1), (2, 4), (3, 1), (4, 1)]), 3))
    expected = [(1, 1), (2, 2)]
    assert actual == expected


# def test_outer_join() -> None:
#     def on(n: Left | Right) -> int:
#         return n.sound_id

#     actual = to_zset(left_table).outer_join(
#         to_zset(right_table), on_left=on, on_right=on
#     )
#     expected = ZSetPython[Pair[Left, Right | None]](
#         {
#             Pair(
#                 left=Left(kind="cow", name="spot", sound_id=4),
#                 right=Right(sound_id=4, sound="moo"),
...
#             ): 1,
#             Pair(
#                 left=Left(kind="ant", name="teeny", sound_id=3),
#                 right=None,
#             ): 1,
#         },
#     )
